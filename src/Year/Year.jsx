                ...
                <div className="year-row" key={week}>
                    {
                        Array(7).fill(0).map((n, i) => {
                            ...
                            let todaySelected = this.state.today === current.format('YYYYMMDD') ? 'year-selected' : '';
                            let isSelected = (Number(this.props.nowDay) === Number(current.format('D')) && Number(this.props.yearStorage) === this.props.nowYear && this.props.nowWeek === week) ? 'yearSelected' : ''
                            let isGrayed = current.format('MM') === today.format('MM') ? '' : 'year-grayed';
                            let clicking = current.format('MM') === today.format('MM') ? this.props.onClickDayInYear : ((e) => e.preventDefault());
                            const day = []
        ...
        return (
            <>
                ...
                <div className='year-component'>
                    <div style={{ color: 'red', fontSize: '30px' }}>{this.props.nowYear}</div>
                    <this.generateWrap />
                </div>
            </>
        )
