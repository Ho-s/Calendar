				...
				<div className='row' key={week}>
					<div onClick={onClickWeek} className={`cw ${thisWeekConst} ${weekSelected}`}>{week}</div>
					{
						Array(7).fill(0).map((n, i) => {
							...
							const todaySelect = m.format('YYYYMMDD') === current.format('YYYYMMDD') ? 'today' : ''
							const isSelected = (Number(nowDay) === Number(current.format('D')) && Number(yearStorage) === nowYear && monthStorage === nowMonth) ? 'selected' : ''
							const clicking = current.format('MM') === m.format('MM') ? onClickDay : ((e: any) => e.preventDefault())
							const isGrayed = current.format('MM') === m.format('MM') ? '' : 'grayed'
