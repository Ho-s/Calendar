...
class Calendar extends PureComponent {
    ...
    generate = () => {
        ...
        for (let week = startWeek; week <= endWeek; week++) {
            ...
            let weekSelected = (Number(this.props.nowWeek) === week && Number(this.props.yearStorage) === this.props.nowYear && this.props.monthStorage === this.props.nowMonth) ? 'selected' : ''
            calendar.push(
                <div className='row' key={week}>
                    <div onClick={this.onClickWeek} className={`cw ${thisWeek} ${weekSelected}`}>{week}</div>
                    {
                        Array(7).fill(0).map((n, i) => {
                            let current = today.clone().week(week).startOf('week').add(n + i, 'day')
                            let todaySelect = this.state.today === current.format('YYYYMMDD') ? 'today' : '';
                            let isSelected = (Number(this.props.nowDay) === Number(current.format('D')) && Number(this.props.yearStorage) === this.props.nowYear && this.props.monthStorage === this.props.nowMonth) ? 'selected' : ''
                            let clicking = current.format('MM') === today.format('MM') ? this.onClickDay : ((e) => e.preventDefault());
                            ...
                            if (current.format('MM') === today.format('MM')) {
                                Array(this.props.storage.length).fill().map((v, n) => {
                                    if (this.props.storage[n].year === Number(this.props.nowYear)) {
                                        if (this.props.storage[n].month === Number(this.props.nowMonth + 1)) {
